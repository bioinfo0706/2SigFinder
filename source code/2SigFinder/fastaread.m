...
%
%   FASTAREAD(...,'IGNOREGAPS',true) removes any gap symbol ('-' or '.')
%   from the sequence(s). Default is false.
%
...
%
%   See also EMBLREAD, FASTAWRITE, GENBANKREAD, GENPEPTREAD,
%   MULTIALIGNREAD, MULTIALIGNVIEWER, SEQPROFILE, SEQTOOL.

%   Copyright 2002-2006 The MathWorks, Inc.
%   $Revision: 1.15.4.15 $  $Date: 2007/11/07 18:15:18 $
            ...
            switch(k)
                case 1  % ignore gaps
                    ...
                    if isempty(ignoreGaps)
                        error('Bioinfo:fastaread:IngoreGapsNotLogical',...
                ...
                case 2  % range
                    ...
                    blockRead = true;
                    range = sort(range);
                    blockText = getfileblock(filename,range,'>'); %#ok
            end
        end
    end
end
...
elseif size(filename,1)>1  % is padded string
