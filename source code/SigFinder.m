...
%% load dataset
    infile=['data\xxxxxx.fasta']; % load datasets    
...
%% Calculate the score of each window using the IST-LFS algorithm
   ...
   Iteration=5; % Periods of time that are repeated to select windows whose scores are large enough to 
             % be considered statistically significant.
...
%% Predict genomic island using 2SigFinder from the rest of the genome
 % Parameters
   ...
   Iteration=1; % Periods of time that are repeated to select windows whose scores are large enough to 
             % be considered statistically significant.
   d_feature=4; % Size of selected dynamic genomic signatures
   sliding_step=10; % The following continued windows to create the ith large sliding window  
