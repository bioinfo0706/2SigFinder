...
%end
%end
% if strcmp(filtration,'3sigma')
% % 		for (chromosome=1 :length(data))
%         [nrow,ncol]=size(data);
% 	    chromOutlayers=cell(1,ncol);
% 		for tetranucleotide=1:ncol
% 		    TetraOutlayers1=(find(data(:,tetranucleotide)>(mean(data(:,tetranucleotide)+3*std(data(:,tetranucleotide))))))';
% 		    TetraOutlayers2=(find(data(:,tetranucleotide)<(mean(data(:,tetranucleotide)-3*std(data(:,tetranucleotide))))))';
% 		    chromOutlayers{tetranucleotide}=[TetraOutlayers1,TetraOutlayers2];
%         end
% 	   outlayers=chromOutlayers;
% %     end
